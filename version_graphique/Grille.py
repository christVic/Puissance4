...
class Grille:
    ...
    def dessiner(self,surface,position_grille,largeur_grille,hauteur_grille):
        """dessine une grille
        Args:
        - surface : surface sur laquelle on dessine la grille
        - position_grille (int,int) : coordonnées x et y de la grille sur la surface
        - largeur_grille (int) : la largeur de la grille
        - hauteur_grille (int) : la hauteur de la grille
        """
        largeur_case = largeur_grille / self.nb_colonnes
        hauteur_case = hauteur_grille / (self.nb_lignes+1)

        largeur_jeton = largeur_case * 0.5
        hauteur_jeton = hauteur_case * 0.5

        """position_numero_colonne_y = hauteur_grille# + hauteur_jeton/8
        position_numero_colonne = (position_grille[0],position_numero_colonne_y)
        surface_numero_colonne = pygame.Surface((largeur_grille,hauteur_case))
        surface_numero_colonne.fill(COULEUR_BLEUE)
"""
        surface_grille = pygame.Surface((largeur_grille,hauteur_grille))
        surface_grille.fill(COULEUR_BLEUE)

        # on affiche la grille
        self.grille.reverse()
        for ligne in range(self.nb_lignes):
            for colonne in range(self.nb_colonnes):
                # on calcule la position du centre du jeton
                position_jeton_y = (ligne * largeur_case) + largeur_jeton
                position_jeton_x = (colonne * largeur_case) + hauteur_jeton
                self.grille[ligne][colonne].dessiner(surface_grille,(position_jeton_x,position_jeton_y),(largeur_jeton,hauteur_jeton))
        self.grille.reverse()

        # on affiche les numeros de colonne
        for colonne in range(self.nb_colonnes):
            # on ecrit le numero de colonne
            posX = (colonne * largeur_case) + largeur_jeton*0.75
            posY =  (self.nb_lignes * hauteur_case) + hauteur_jeton/2
            police = pygame.font.SysFont(None,40)
            numero_texte = police.render(str(colonne+1),True,(COULEUR_BLANCHE))
            surface_grille.blit(numero_texte,(posX,posY))

        surface.blit(surface_grille,position_grille)
